...
def load_database (path: str) -> dict[str, dict]:
    try:
        with open(path) as f:
            database = json.load(f) # type: ignore

    except FileNotFoundError:
        database = {}
    return database

def save_databases(database: dict[str, dict], path: str) -> None:
...
def main() -> None:
    ...
    DB_PATH: str = os.path.expanduser("~/taskly.json")
    database = load_database(DB_PATH)
    try:
        querie(database, **args) # ➜ add_task(description="hacer mercado")
    except KeyError:
        sys.exit("No task found with the provided ID")
    save_databases(database, DB_PATH)